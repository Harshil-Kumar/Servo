...
"use client";
import { getClientsPrevious } from "@/app/actions/clientActions/presentClients";
...
interface PreviousClientsProps {
  clients?: Client[]; // Make clients prop optional
}

const PreviousClients: React.FC<PreviousClientsProps> = ({ clients = [] }) => {
  const [clientsMongo, setClientsMongo] = useState<ClientMongo[]>([]);
  const [search, setSearch] = useState("");
  const [options, setOptions] = useState<ClientMongo[]>([]);
