...
import PreviousClients from "../previous-clients/page";
import {
  addClient,
  getClients,
  removeClient,
  tranferToPrevious,
  updateClient,
} from "@/app/actions/clientActions/presentClients";
...
const PresentClients = () => {
  ...
  const [clientsMongo, setClientsMongo] = useState<ClientMongo[]>([]);
  const [clientsSelect, setClientsSelect] = useState<ClientMongo[]>([]);
  const [editBool, setEditBool] = useState<Boolean>(false);
  const [newClient, setNewClient] = useState<ClientMongo>({
    _id: "",
    clientName: "",
    email: "",
    contactPerson: "",
  ...
  return (
    <>
      ...
      >
        Transfer to Previous Clients
      </button>
    </>
  );
};

export default PresentClients;